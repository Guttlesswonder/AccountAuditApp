...
export function OpportunityRegister({ responses, adoption, platforms }: {
  ...
  return (
    <div className="rounded border bg-white p-3">
      ...
      <div className="space-y-2 text-sm">{rows.map((r, i) => <div key={`${r.title}-${i}`} className="rounded border p-2"><p className="font-medium">{r.title}</p><p>{r.platform} • {r.category} • confidence: {r.confidence}</p><p>{r.whyItFits}</p>{r.nextStep && <p>Next step: {r.nextStep}</p>}<p className="text-xs text-slate-600">Evidence: {r.linkedEvidence}</p></div>)}</div>
    </div>
  );
}
